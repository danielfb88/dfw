...
abstract class DAO {
    ...
    protected function __construct() {
        ...
        if (empty($this->tableName)) {
            throw $e = new DAOException('Não há uma tabela definida na subclasse ' . $this->className);
            $e->getTraceAsString();
        }

        ## Excessão para CAMPO DA PRIMARY KEY INEXISTENTE ##
        if (!$this->configProps['primaryKey']['field']) {
            throw $e = new DAOException('Não há primary key definida na subclasse ' . $this->className);
            $e->getTraceAsString();
        }
        ...
        if (!$this->properties) {
            throw $e = new DAOException('Os campos da tabela não foram definidos na subclasse.');
            $e->getTraceAsString();
        }
    ...
    private function selectSQL(array $filterValues, $whereType, $orderBy = null, $useBindValue = false) {
        if ($whereType != '=' && $whereType != 'like') {
            throw $e = new DAOException('Parâmetro whereType inválido. Use \'=\' ou \'like\'');
            $e->getTraceAsString();
        }
        ...
        if ($orderBy && is_string($orderBy)) {
            ...
            if (!$orderByFound) {
                throw $e = new DAOException("O parâmetro orderBy informado não existe");
                $e->getTraceAsString();
            }
        }
    ...
    private function insertSQL(array $insertValues, $useBindValue = false) {
        ...
        if (!$insertValues && !is_array($insertValues)) {
            throw $e = new DAOException('Parâmetro passado para insert inválido.');
            $e->getTraceAsString();
        }
    ...
    private function updateSQL(array $updateValues, $useBindValue = false) {
        ...
        if (!$updateValues && !is_array($updateValues)) {
            throw $e = new DAOException('Parâmetro passado para update inválido.');
            $e->getTraceAsString();
        }

        // o valor da primaryKey nao foi informado
        if (!$this->checkValuePrimaryKeyExists())
            throw $e = new DAOException('O valor da primarykey não foi informado');
    ...
    public function read($useBindValue = true) {
        ...
        if (!$filterValues) {
            throw $e = new DAOException('Nenhum filtro foi especificado para o read()');
            $e->getTraceAsString();
        }
    ...
    public function insert($useBindValue = true) {
        ...
        if (!$this->checkValuesNotNullFieldsExists()) {
            ...
            throw $e = new DAOException('Alguns dos campos ' . $fields . ' definidos como NotNull estão vazios.');
            $e->getTraceAsString();
        }
